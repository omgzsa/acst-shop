...
const { data: techSpec } = await useAsyncData('techSpec', () => {
  return getItemById({
    collection: `kerekparosEsSportTechSpec`,
    id: data.value.kerekparosEsSportTechSpec[0].id,
    filter: {
      ...
      <div class="grid lg:grid-cols-3 gap-x-6 gap-y-10 site-padding">
        ...
        <ProductTechSpec v-if="!noTechSpec">
          ...
          <ProductTechSpecItem
            name="Vízállóság"
            type="string"
            :value="techSpec.vizallosag"
            v-if="techSpec.vizallosag !== null"
          />
          <ProductTechSpecItem
            name="Anyaga"
            type="string"
